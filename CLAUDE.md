...
## Current status: home page on real data — `M24`+ is next

Planning and documentation are **Done** (see [docs/TASKS.md](./docs/TASKS.md)). Implementation has progressed through the foundation group, the full Payload data model, the auth confirmation pass, the last of the legacy route removal, and the first two storefront-data milestones: `M1`, `M2`, `M2a`, `M16`, `M17`, `M19`, `M14`, `M3`, `M4`, `M5`, `M6`–`M13`, `M13a`, `M20`, `M21`, `M15`, `M18`, `M22`, and `M23` are all **Done**. Prisma is retired. A dev Dockerfile exists (its `docker build` step is implemented but not fully verified — see [docs/TASKS.md](./docs/TASKS.md)).

**`lib/payload/products.ts` and `lib/payload/categories.ts`** are the server-side (Local API) data utilities — `getProducts()`, `getFeaturedProducts()`, `getProductById()`, `getTopLevelCategories()`, `getCategoryBySlug()`, `getProductsByCategory()` (with descendant rollup). Server components only; client components should hit the REST/GraphQL API instead. Types in both are hand-written rather than generated from `payload-types.ts` (blocked in this sandbox — see [docs/TASKS.md](./docs/TASKS.md)).

**The home page (`/`) is the only storefront route on real data so far** (`M23`) — a `force-dynamic` server component. `/shop`, `/product/[id]`, and the category marquee are still on `assets/assets.js` dummy data until `M24`, `M25`, and `M27`. Note `"Best Selling"` is an **admin-curated `isFeatured` flag**, not a computed ranking ([ADR-022](./docs/DECISIONS.md)) — there is no sales or review data to rank by.
